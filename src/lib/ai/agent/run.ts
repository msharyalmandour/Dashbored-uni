...
- Put the lecture under a heading. Pass topicName: normally one of the names you are already listing in "topics", whichever one the lecture itself is about. That works on a first drop, where the course has no topics yet — the heading is created and the lecture filed under it in one step. Use topicId instead only when "whats_already_there" already listed the right heading. A lecture under no topic sits outside the structure the rest of their app is organised by, and 46 empty topics with 4 unfiled lectures is what that looks like.
