  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-semibold tracking-tight">Problem Solving</h1>
          <p className="text-sm text-muted-foreground">Practice, self-grade, and let mistakes feed the journal automatically.</p>
        </div>
        <CreateProblemDialog subjects={subjects} defaultSubjectId={sp.subject} />
      </div>

      <div className="grid grid-cols-3 gap-3">
        <StatCard label="Total Problems" value={total} icon={PencilLine} />
        <StatCard label="Correct" value={correct} icon={CheckCircle2} tone="success" />
        <StatCard label="Incorrect" value={incorrect} icon={XCircle} tone="destructive" />
      </div>
