...
export function RepeatedMistakesChart({ data }: { data: { name: string; count: number }[] }) {
  const { sequential, chrome } = useChartTheme();
  if (data.length === 0) return <NoData text="No open mistakes — nothing repeating." />;
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout="vertical" margin={{ top: 4, right: 28, left: 8, bottom: 4 }}>
        ...
        <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8, border: `1px solid ${chrome.grid}` }} formatter={(v) => [v, "Mistakes"]} />
